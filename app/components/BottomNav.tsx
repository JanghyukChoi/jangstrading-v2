...
export default function BottomNav() {
  ...
  return (
    <nav
      className="sm:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#17171c]/95 backdrop-blur-xl border-t border-white/[0.06]"
      ...
      <div className="flex items-center justify-around h-14">
        {items.map((item) => {
          ...
          return (
            ...
            >
              {item.icon}
              <span className={`text-[12px] ${active ? "font-medium" : ""}`}>{item.label}</span>
            </Link>
          );
        })}
      </div>
