...
export default function SectorsPage() {
  ...
  const sectors = useMemo(() => {
    const map: Record<string, { stocks: StockRanking[]; foreign: number; institution: number; combined: number; totalCap: number }> = {};
    for (const s of allStocks) {
      const sector = s.sector || "기타";
      if (!map[sector]) map[sector] = { stocks: [], foreign: 0, institution: 0, combined: 0, totalCap: 0 };
      map[sector].stocks.push(s);
      map[sector].foreign += s.foreign[period] ?? 0;
      map[sector].institution += s.institution[period] ?? 0;
      map[sector].combined += s.combined[period] ?? 0;
      map[sector].totalCap += s.market_cap ?? 0;
    }

    const result: SectorData[] = Object.entries(map).map(([name, data]) => {
      const netPurchase = investor === "foreign" ? data.foreign : investor === "institution" ? data.institution : data.combined;
      const ratio = data.totalCap > 0 ? Math.round(netPurchase / data.totalCap * 1000) / 10 : null;
      return {
        name,
    ...
    return result;
  }, [allStocks, investor, period, sortBy]);
          ...
          return (
            ...
            >
              ...
              <span className="flex-1 min-w-0 flex items-center gap-1.5">
                <span className="text-white font-medium text-[12px] sm:text-[13px]">{s.name}</span>
                <span className="text-[10px] text-[var(--text-muted)] hidden sm:inline">{s.stockCount}</span>
