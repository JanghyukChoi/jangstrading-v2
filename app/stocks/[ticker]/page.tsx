...
function CNum({ v, size = "text-sm", short = false }: { v: number; size?: string; short?: boolean }) {
  const cls = v > 0 ? "positive" : v < 0 ? "negative" : "text-[var(--text-secondary)]";
  const display = short ? fmtShort(v) : fmt(v);
  return <span className={`num ${cls} ${size}`}>{v > 0 ? "+" : ""}{display}</span>;
}
          ...
          <tbody>
            {periods.map((p) => (
              <tr key={p} className="border-t border-white/[0.03]">
                ...
                <td className="py-2.5 text-right"><CNum v={stockData.foreign[p]} short /></td>
                <td className="py-2.5 text-right"><CNum v={stockData.institution[p]} short /></td>
                <td className="py-2.5 text-right font-medium"><CNum v={stockData.combined[p]} short /></td>
              </tr>
            ))}
          </tbody>
        </table>
